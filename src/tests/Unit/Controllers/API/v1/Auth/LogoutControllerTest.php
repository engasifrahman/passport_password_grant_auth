...
class LogoutControllerTest extends TestCase
{
    ...
    private function createAndMockLogoutRequest(bool $shouldTokenDeletionFail): Request
    {
        ...
        if ($shouldTokenDeletionFail) {
            ...
            $token = Mockery::mock();
            $refreshToken = Mockery::mock();

            // Expect revoke on refreshToken
            $refreshToken->shouldReceive('revoke')->once()->andReturn(true);

             // Expect revoke on token
            $token->shouldReceive('revoke')->once()->andReturn(true);

            // Link refreshToken to token
            $token->refreshToken = $refreshToken;

            // Mock currentAccessToken() to return our fake token
